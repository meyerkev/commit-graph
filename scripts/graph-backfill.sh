...
make_commit_at() {
  ...
  # Create an empty commit if we're removing seed files
  if ! [[ "$msg" =~ "remove graph seed files" ]]; then
...
while :; do
  ...
  need=$target
  if [ "$need" -le 0 ]; then
    log "$current_day: target=$target, need=$need — skip"
  else
    log "$current_day: target=$target, need=$need"
